...
import com.mycompany.repository.IKhuyenMaiRepository;
 ...
 * @author Duongntt
 */
public class KhuyenMaiRepository implements ICommonRepository<KhuyenMai, Boolean, String>, IKhuyenMaiRepository {
    ...
    public Boolean remove(String ma) {
        ...
        try ( Session session = HibernateUtil.getFactory().openSession()) {
            ...
            transaction.begin();
            try {
                Query query = session.createQuery("UPDATE KhuyenMai SET trangThai = 1 WHERE maKhuyenMai = :maKM");
                query.setParameter("maKM", ma);
                check = query.executeUpdate();
                transaction.commit();
            } catch (Exception e) {
...
//        System.out.println(khuyenMai1.toString());
        List<KhuyenMai> list = new KhuyenMaiRepository().searchKM("");
    ...
    public List<KhuyenMai> searchKM(String keyWord) {
        List<KhuyenMai> listKM = new ArrayList<>();
        try ( Session session = HibernateUtil.getFactory().openSession()) {
            Query query = session.createQuery("FROM KhuyenMai WHERE tenKM LIKE :keyWord");
